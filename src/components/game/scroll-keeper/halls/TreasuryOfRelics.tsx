    ...
    <div className="fixed inset-0 bg-gradient-to-b from-slate-950 via-yellow-950/20 to-slate-950 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="absolute w-24 h-32 md:w-32 md:h-44 rounded-lg border-2 border-yellow-800/30 bg-slate-900/30 backdrop-blur-sm"
              style={{
                left: `${10 + i * 15}%`,
                top: `${20 + (i % 2) * 40}%`,
            ...
            >
              <div className="w-full h-full flex items-center justify-center">
                <HelpCircle className="w-8 h-8 text-yellow-900/30" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="absolute top-4 right-4 z-50">
        <div className={cn(
          ...
          <Clock className={cn("w-5 h-5", timer <= 10 ? "text-red-400 animate-pulse" : "text-yellow-400")} />
          <span className={cn("text-xl font-mono font-bold", timer <= 10 ? "text-red-400" : "text-yellow-200")}>
            {timer}с
          </span>
        </div>
      </div>
      ...
      <div className="absolute top-4 left-4 z-50">
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-yellow-900/50 border border-yellow-600/50 backdrop-blur-sm">
          <span className="text-2xl">⚱️</span>
          <span className="text-yellow-200 font-medium">Сокровищница</span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-8">
        <div className="max-w-2xl w-full space-y-8">
          ...
          <div className="relative mx-auto w-64 h-80 md:w-80 md:h-96">
            ...
            <div className="absolute inset-0 border-4 border-yellow-600/60 rounded-xl bg-gradient-to-b from-slate-900/60 via-slate-800/40 to-slate-900/60 backdrop-blur-md shadow-[0_0_60px_rgba(234,179,8,0.2)]">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className={cn(
                  ...
                  <div className="w-24 h-32 md:w-32 md:h-40 bg-gradient-to-b from-yellow-500/20 via-yellow-600/10 to-transparent rounded-lg blur-sm animate-pulse" />
                  <Sparkles className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 md:w-20 md:h-20 text-yellow-400/40" />
                </div>
              </div>
            ...
            {/* Pedestal */}
            <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-48 md:w-56 h-8 bg-gradient-to-b from-yellow-800 to-yellow-900 rounded-lg shadow-lg">
          ...
          <div className="space-y-4 mt-8">
            {sentences.slice(0, revealProgress).map((sentence, i) => (
              <div 
                key={i}
                className="bg-slate-900/60 backdrop-blur-sm px-6 py-4 rounded-xl border border-yellow-600/30 animate-fade-in"
            ...
            {revealProgress < sentences.length && (
              <div className="flex items-center justify-center gap-2 py-4 text-yellow-400/60">
          ...
          {challenge.relatedEvents && showInput && (
            <div className="flex flex-wrap justify-center gap-2 animate-fade-in">
              {challenge.relatedEvents.map((event, i) => (
                <span 
                  key={i}
                  className="px-3 py-1 rounded-full bg-yellow-900/40 border border-yellow-600/30 text-yellow-300/70 text-sm"
                >
                  {event}
                </span>
              ))}
          ...
          {showInput && (
            <div className="flex gap-4 animate-fade-in">
              <Input
                ...
                className="flex-1 bg-yellow-950/50 border-yellow-600/50 text-white text-lg py-6 placeholder:text-yellow-300/50 focus:border-yellow-500"
                autoFocus
              />
              <Button 
                ...
                size="lg"
                className="bg-yellow-600 hover:bg-yellow-700 text-white px-8 shadow-[0_0_20px_rgba(234,179,8,0.3)]"
              >
