  ...
  return (
    <div className="fixed inset-0 bg-slate-950 overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-50 flex items-center gap-2">
        <button
          className="flex items-center justify-center w-12 h-12 rounded-full bg-slate-800/50 border border-slate-600/50 backdrop-blur-sm hover:bg-slate-700/50 transition-colors"
        >
        ...
        <div className={cn(
          ...
          <span className={cn("text-2xl font-mono font-bold", timer <= 10 ? "text-red-400" : "text-white")}>
            {timer}с
          </span>
        </div>
      </div>
      ...
      <div className="absolute top-4 left-4 z-50">
        <div className="flex items-center gap-3 px-5 py-3 rounded-full bg-slate-800/50 border border-slate-600/50 backdrop-blur-sm">
          <span className="text-3xl">🎭</span>
          <span className="text-slate-300 font-semibold text-lg">Зал Теней</span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-8">
        <div className="max-w-3xl w-full space-y-8">
          ...
          <div className="relative">
            ...
            <div className="relative bg-slate-900/80 p-10 rounded-xl backdrop-blur-sm">
              {/* Story text */}
              <div 
              ...
              >
                <p className="text-slate-200 text-xl md:text-2xl leading-relaxed">
                  ...
                  {isTyping && (
                    <span 
                      className={cn(
                        "inline-block w-0.5 h-6 ml-1 bg-amber-400 transition-opacity duration-100",
              ...
              {showQuestion && (
                <div className="mt-6 pt-6 border-t border-slate-700 animate-fade-in">
                  <p className="text-amber-400 text-2xl md:text-3xl font-medium text-center">
                    {challenge.question}
                  </p>
                </div>
              )}
            ...
            <div className="space-y-3">
              {usedHints > 0 && (
                <div className="space-y-2">
                  {challenge.hints.slice(0, usedHints).map((hint, i) => (
                    <div 
                      key={i} 
                      className="flex items-center gap-3 text-slate-400 bg-slate-800/50 px-5 py-3 rounded-lg animate-fade-in"
                    >
              ...
              {usedHints < challenge.hints.length && (
                <Button 
                  ...
                  size="lg"
                  onClick={onUseHint}
                  className="border-slate-600 text-slate-400 hover:bg-slate-800 hover:text-slate-300 text-base"
                >
