...
const StoryImageSlide = ({ slide, direction }: StoryImageSlideProps) => {
  ...
  return (
    <div className="min-h-screen w-full relative">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-amber-950/85 via-amber-950/50 to-amber-950/20" />
                ...
                <div className="star-wars-crawl crawl-fade w-full overflow-x-hidden">
                  <div className={`crawl-content px-[20rem] ${animationStarted ? 'animate' : ''}`}>
                    {slide.story.map((paragraph, index) => (
                      <p 
                        key={index} 
                        className="text-4xl md:text-5xl lg:text-6xl text-amber-100 font-serif leading-relaxed mb-10 whitespace-normal break-words text-justify text-justify"
                      >
                        {paragraph}
                      </p>
                    ))}
